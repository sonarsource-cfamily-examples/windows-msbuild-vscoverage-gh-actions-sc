= C++ example project scanned on SonarCloud with code coverage using GitHub Actions
// URIs:
...
*This project is analysed on https://sonarcloud.io/dashboard?id=sonarsource-cfamily-examples_windows-msbuild-vscoverage-gh-actions-sc[SonarCloud]!*
...
.. Run `sonar-scanner` as the final step with the extra property `sonar.cfamily.vscoveragexml.reportsPath` set with the path that will contain your XML coverage report
